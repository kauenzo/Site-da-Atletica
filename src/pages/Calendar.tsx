...
const getEventTypeColor = (type: string) => {
  switch (type) {
    case "treino": return "bg-atletica-red";
    case "campeonato": return "bg-atletica-burgundy";
    case "torneio": return "bg-atletica-sand";
    case "reuniao": return "bg-atletica-black";
    default: return "bg-atletica-red";
  }
};
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            {(selectedEvents.length > 0 ? selectedEvents : events.slice(0, 4)).map((event) => (
              <Card key={event.id} className="bg-atletica-white shadow-button hover-lift">
                <CardContent className="p-6 space-y-3">
                  <div className="flex items-start justify-between">
                    <h3 className="font-semibold text-atletica-burgundy">
                      {event.title}
                    </h3>
                    <Badge className={`${getEventTypeColor(event.type)} text-atletica-white`}>
                      {event.type}
                    </Badge>
                  </div>
